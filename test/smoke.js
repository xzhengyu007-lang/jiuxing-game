...
assert(S.cndTotal===1,'凝聚应计入累计凝聚, 实际 '+S.cndTotal);
const atk9a=heroStats().atk;
condense(1);condense(1);
assert(S.cnd.n===3,'再凝两道应为3, 实际 '+S.cnd.n);
assert(heroStats().atk>atk9a,'凝聚应增攻防气血战力');
S.qi=0;
condense(99);
...
assert(S.cndTotal===13,'累计凝聚应为13, 实际 '+S.cndTotal);
assert(S.qi>=layerCost(),'凝聚后应余冲击灵气');
tryBreak();
assert(S.g===g9+1,'凝聚圆满后应突破, g='+S.g);
cnd();
assert(S.cnd.n===0&&S.cnd.l===curL()&&S.cnd.r===curR(),'换层后凝聚进度应重置');
assert(S.cndTotal===13,'换层后累计凝聚不应清零');
const pnl9=charStatPanelHtml();
assert(pnl9.indexOf('暴击')>=0&&pnl9.indexOf('关窍凝聚')>=0,'人物属性面板应含八维与凝聚行');
Math.random=rr9;
curTab='cult';renderAll();
/* ========== 阶段十：魂修 / 火焰 / 丹炉 / 珠内炼丹 ========== */
S=newState();S.ap=999999;
assert(FLAMES.length===400,'应有400种火焰');
assert(FLAME_CLASSES.length===4,'应为四大丹火');
assert(FLAMES_BY_CLS[0].length===100&&FLAMES_BY_CLS[1].length===100&&FLAMES_BY_CLS[2].length===100&&FLAMES_BY_CLS[3].length===100,'每类应100种');
assert(FLAMES_BY_CLS[0][0].rank===1&&FLAMES_BY_CLS[0][0].pow>FLAMES_BY_CLS[0][50].pow,'兽火榜第1名应最强');
assert(FLAMES_BY_CLS[3][0].pow>FLAMES_BY_CLS[2][0].pow,'神火榜首应强于天火榜首');
assert(FURNACES.length===8,'应为八阶丹炉');
assert(soulRankIdx()===0,'初始应为淬魂');
S.qi=layerCost()*100;
soulRaise();
assert(S.soul.exp>0,'养魂应得灵魂之力');
S.soul.exp=800;
assert(soulRankIdx()===1,'灵魂之力800应为凝魂');
S.stones=1e9;
buySoulGf('sg1');
assert(S.soul.gfs.sg1,'应购得淬魂诀');
assert(soulGfFx('soulexp')===0.25,'淬魂诀应加养魂25%');
igniteNatal();
assert(S.natal&&S.natal.pow>0,'应点燃本命之火');
const np10=S.natal.pow,se10=S.soul.exp;
S.soul.exp=2000;
S.flames['b_1']=1;
devourFlame('b_1');
assert(!S.flames['b_1'],'吞噬后异火应消失');
assert(S.natal.pow>np10,'吞噬应涨本命火力');
assert(S.soul.exp>se10,'吞噬应涨灵魂之力');
S.furnace=0;
assert(fireGate('r_puti')!=='','位阶8丹方应有丹火门槛');
S.furnace=7;
assert(fireGate('r_puti')!=='','炉够而火不够仍应拦');
S.natal={cls:3,rank:1,n:'混沌神火',pow:20000};
assert(fireGate('r_puti')==='','炉火皆足应放行');
assert(fireGate('huiqi')==='','低阶丹方不应设门槛');
S.pearlAlch.rid='huiqi';
S.danExp=1e4;
const cap10=pearlCap('huiqi');
assert(cap10>=1,'每日珠内炼丹至少1炉');
S.herbs.lingcao=999;
pearlCraftOnce();
assert(S.pearlAlch.used===1,'珠内炼一炉应计数');
S.pearlAlch.auto=1;
pearlDaily();
assert(S.pearlAlch.used>0,'次日自动炼丹应执行');
S.soul.exp=16000;
assert(soulRankIdx()===3,'16000应为魂变');
const ss10=SOUL_SKILLS.find(x=>x.id==='souls_zhua');
assert(ss10&&skillOk(ss10),'魂变阶应用九幽魂爪');
S.soul.exp=100;
assert(!skillOk(ss10),'魂力不足魂技应不可用');
